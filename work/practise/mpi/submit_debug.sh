...
sh ${HPC_HOME}/bin/qsub_f --hdfs ${HADOOP_FS} --ugi ${HADOOP_UGI} --pushcore no --core-size-limit 8999 --hpcugi ${HADOOP_UGI} --hout ${HADOOP_OUT} --files ${CURR_DIR}/vectorcompute_debug.o -N chenkang02_practise -l nodes=${NUM_NODES},walltime=72000 ${CURR_DIR}/run_mpi_debug.sh 
